...
if not os.path.exists(os.path.join(args.intermediate_dir, args.domain, 'query_retrieval', 'dpr')):
    os.mkdir(os.path.join(args.intermediate_dir, args.domain, 'query_retrieval', 'dpr'))
print(os.path.join(args.intermediate_dir, args.domain, 'query_retrieval', 'dpr'))

save_path = os.path.join(args.intermediate_dir, args.domain, 'query_retrieval', 'dpr')
...
for index, row in tqdm(df_all['train'].iterrows()):
    if 'positives' not in train_data[row['query_id']]:
        train_data[row['query_id']]['query'] = row['query']
        train_data[row['query_id']]['positives'] = []
        train_data[row['query_id']]['negatives'] = []
    train_data[row['query_id']]['positives'].append(row['product_id'])
    ...
    for line in tqdm(readin):
            ...
            try:
                if int(rank) > args.max_rank:
                    continue
                if docid not in set(train_data[int(qid)]['positives']):
                    train_data[int(qid)]['negatives'].append(docid)
            except:
                embed()

for qid in tqdm(train_data):
    train_data[qid]['positives'] = [text_process(meta_dict[docid]) for docid in train_data[qid]['positives']]
    train_data[qid]['negatives'] = [text_process(meta_dict[docid]) for docid in train_data[qid]['negatives']]
...
for index, row in tqdm(df_all['val'].iterrows()):
    if 'positives' not in val_data[row['query_id']]:
        val_data[row['query_id']]['query'] = row['query']
        val_data[row['query_id']]['positives'] = []
        val_data[row['query_id']]['negatives'] = []
    val_data[row['query_id']]['positives'].append(row['product_id'])
...
with open(os.path.join(args.bm25_dir, 'val.bm25.trec')) as f:
    ...
    for line in tqdm(readin):
            ...
            try:
                if int(rank) > args.max_rank:
                    continue
                if docid not in set(val_data[int(qid)]['positives']):
                    val_data[int(qid)]['negatives'].append(docid)
            except:
                embed()

for qid in tqdm(val_data):
    val_data[qid]['positives'] = [text_process(meta_dict[docid]) for docid in val_data[qid]['positives']]
    val_data[qid]['negatives'] = [text_process(meta_dict[docid]) for docid in val_data[qid]['negatives']]


## save
print(f'-------- Save --------')

with open(os.path.join(save_path, 'train.text.jsonl'), 'w') as fout:
    for qid in tqdm(train_data):
        tmp = train_data[qid]
        fout.write(json.dumps(tmp)+'\n')

with open(os.path.join(save_path, 'val.text.jsonl'), 'w') as fout:
    for qid in tqdm(val_data):
        tmp = val_data[qid]
        fout.write(json.dumps(tmp)+'\n')
...
doc2id = defaultdict(int)
with open(os.path.join(args.bm25_dir, 'documents.json')) as f:
    with open(os.path.join(save_path, 'corpus.tsv'), 'w') as fout:
        docs = json.load(f)
        for docid in docs:
            if docid not in doc2id:
                doc2id[docid] = len(doc2id)
            fout.write(str(doc2id[docid]) + '\t' + text_denoise(docs[docid]) + '\n')
...
with open(os.path.join(args.bm25_dir, 'test.truth.trec')) as f:
    with open(os.path.join(save_path, 'test.truth.trec'), 'w') as fout:
        ...
        for line in readin:
            ...
            fout.write(qid + ' ' + str(0) + ' ' + str(doc2id[docid]) + ' ' + str(1) + '\n')
