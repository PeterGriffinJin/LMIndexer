...
class TrainDataset(Dataset):
    ...
    def __init__(self, tokenizer, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(TrainDataset, self).__init__()

        tokenizer_name = args.model_name.split('/')[-1]
        self.data_file = os.path.join(args.data_dir, 'document.tokenized.debug.json')
        dataset = load_dataset("json", data_files=self.data_file, streaming=False, cache_dir=cache_dir)["train"]
        self.dataset = dataset.shuffle(seed=shuffle_seed) if shuffle_seed is not None else dataset
    ...
    def process_fn(self, example):
        ...
        encoded_query = self.create_one_example(example['token_ids'])
...
data = TrainDataset(tokenizer, shuffle_seed=None)
...
with torch.no_grad():
    for batch_data in tqdm(dataloader):
        ...
        elif args.model_name in ['t5-base']:
