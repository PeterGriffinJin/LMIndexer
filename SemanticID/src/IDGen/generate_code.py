...
from IDGen.modeling import SemanticIDLM, SemanticIDQELM
from IDGen.trainer import DenseIDTrainer
from IDGen.utils import calculate_metrics, save_code_metrics
...
def main():
    ...
    eval_dataset = EvalIDDataset(tokenizer, data_args, shuffle_seed=None, cache_dir=data_args.data_cache_dir or model_args.cache_dir) if data_args.eval_path is not None else None
    ...
    trainer = trainer_cls(
        ...
        compute_metrics=save_code_metrics,
    )
    ...
    # save codes
    with open(os.path.join(model_args.save_id_dir), 'w') as fout:
        for idd in metrics['eval_codes'].tolist():
            fout.write(str(idd)+'\n')
